...
import { escapeHtml, convertPhpToUsd } from './utils';
      ...
      html: `
        <div style="font-family: system-ui, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
          ...
          <div style="background: #f5f5f5; padding: 20px; border-radius: 12px; margin: 20px 0;">
            <p><strong>Type:</strong> ${escapeHtml(commission.artType)}</p>
            ${commission.style ? `<p><strong>Style:</strong> ${escapeHtml(commission.style)}</p>` : ''}
            ${commission.estimatedPrice ? `<p><strong>Estimated Price:</strong> ₱${commission.estimatedPrice} (~$${convertPhpToUsd(commission.estimatedPrice)} USD)</p>` : ''}
          </div>
